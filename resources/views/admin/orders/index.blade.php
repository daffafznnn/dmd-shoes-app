<x-app-layout>
    <div class="container mx-auto px-6 py-8">
        ...
        <!-- Title Section -->
        <div class="mb-6">
            <h1 class="text-3xl font-semibold text-gray-800 dark:text-white">{{ __('Daftar Order') }}</h1>
        </div>
        ...
        <div class="mb-6">
            <div class="card bg-white shadow-lg border dark:border-primary-darker dark:bg-darker">
                <div class="card-body">
                    <h2 class="card-title mb-4 text-gray-800 dark:text-white">{{ __('Filter Order') }}</h2>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                            <div class="col-span-2 lg:col-span-3">
                                ...
                                <input type="text" name="search" id="search" class="input input-bordered w-full bg-white dark:bg-darker"
                                    placeholder="{{ __('Cari kode order') }}"
                            ...
                            <div>
                                ...
                                <select name="status" id="status" class="select select-bordered w-full bg-white dark:bg-darker">
                                    ...
                                    <option value="pending" {{ request()->query('status') == 'pending' ? 'selected' : '' }}>{{ __('Menunggu') }}</option>
                                    <option value="processing" {{ request()->query('status') == 'processing' ? 'selected' : '' }}>{{ __('Proses') }}</option>
                                    <option value="completed" {{ request()->query('status') == 'completed' ? 'selected' : '' }}>{{ __('Selesai') }}</option>
                                    <option value="cancelled" {{ request()->query('status') == 'cancelled' ? 'selected' : '' }}>{{ __('Batal') }}</option>
                                </select>
                            </div>
                        </div>
                        <div class="flex justify-end mt-4">
                            <button type="submit" class="btn btn-primary">{{ __('Filter') }}</button>
                            <a href="{{ route('master.orders.index') }}" class="btn btn-secondary ml-2">{{ __('Reset') }}</a>
                        </div>
                    </form>
                </div>
            </div>
                ...
                @else
                    <table class="table w-full">
                        ...
                        <tbody>
                                ...
                                <tr class="hover:bg-gray-100 dark:hover:bg-gray-700">
                                   ...
                                   <td>
                                        <span class="badge {{ 
                                            ...
                                            {{ 
                                                $order->status === 'pending' ? __('Menunggu') : 
                                                ($order->status === 'processing' ? __('Proses') : 
                                                ($order->status === 'completed' ? __('Selesai') : 
                                                __('Batal'))) 
                                            }}
                                        </span>
                                    </td>
                                    <td>
                                        <div class="flex space-x-2">
                                            <a href="{{ route('master.orders.show', $order->id) }}" class="btn btn-accent btn-xs">{{ __('Detail') }}</a>
                                            <button onclick="restoreOrder('{{ route('master.orders.restore', ['id' => $order->id]) }}')"
                                                    class="btn btn-secondary btn-xs">
                                                    {{ __('Restore') }}
                                            </button>
                                            <button onclick="deleteOrder('{{ route('master.orders.destroy', ['id' => $order->id]) }}')"
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" class="text-center py-4 text-gray-800 dark:text-gray-300">{{ __('Tidak ada order ditemukan.') }}</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
