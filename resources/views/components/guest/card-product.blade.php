...
<div
    ...
    <div class="mt-4 px-5 pb-5">
        <h5 class="text-xl tracking-tight text-gray-900">{{ $product->name }}</h5>
        <p class="text-sm text-gray-500">{{ $product->type }}</p>
        <div class="mt-2 mb-5 flex items-center justify-between">
            <p>
                <span class="text-xl font-semibold text-gray-700">Rp
        ...
        <a href="https://wa.me/{{ substr($setting->phone, 1) }}?text={{ urlencode(__('Checkout this product') . __(': :name at Rp :price', ['name' => $product->name, 'price' => number_format($product->default_price, 0, ',', '.')])) }}"
            target="_blank"
            class="flex items-center justify-center bg-gray-950 px-2 py-1 text-sm text-white transition hover:bg-black">
